...
class VirtualFileSystem {
  ...
  statSync(filePath, options) {
    const providerPath = this.#toProviderPath(filePath);
    return ensureBigIntStats(options, this[kProvider].statSync(providerPath, options));
  }

  lstatSync(filePath, options) {
    const providerPath = this.#toProviderPath(filePath);
    return ensureBigIntStats(options, this[kProvider].lstatSync(providerPath, options));
  }
  ...
  #createPromisesAPI() {
    ...
    return Object.freeze({
      ...
      async stat(filePath, options) {
        const providerPath = toProviderPath(filePath);
        return ensureBigIntStats(options, await provider.stat(providerPath, options));
      },

      async lstat(filePath, options) {
        const providerPath = toProviderPath(filePath);
        return ensureBigIntStats(options, await provider.lstat(providerPath, options));
      },
