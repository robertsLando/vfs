...
const { ERR_INVALID_ARG_VALUE } = require('./errors.js');
...
// Providers differ: RealFSProvider forwards to the real fs and answers with genuine
// BigIntStats, while the memory and sqlite providers have no bigint shape at all.  Checking
// the answer rather than the request keeps the first working and stops the others handing
// back Number fields that break on the caller's first `stats.size > 0n`.
function ensureBigIntStats(options, stats) {
  if (options?.bigint && typeof stats?.size !== 'bigint') {
    throw new ERR_INVALID_ARG_VALUE(
      'options.bigint', options.bigint, 'is not supported by this provider');
  }
  return stats;
}
