...
// There is no BigIntStats shape here, so answering a bigint request with Number fields
// hands back stats that break on the caller's first `stats.size > 0n`.
function rejectBigIntStats(options) {
  if (options?.bigint) {
    throw new ERR_INVALID_ARG_VALUE(
      'options.bigint', options.bigint, 'is not supported by a virtual file system');
  }
}
