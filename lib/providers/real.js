...
// Captured before installFsPatches replaces them.  RealFSProvider *is* the real filesystem,
// so its own calls have to reach the kernel: going through the patched members re-enters the
// mount table that routes here, so a VFS mounted on this provider's root serves its content
// instead of the disk, #resolvePath's containment check never reaches the open it validated,
// and a provider mounted at its own root recurses until the stack overflows.
// The promises members are captured one by one, not as the namespace: installFsPatches
// replaces them in place on the shared fs.promises object, so holding a reference to the
// namespace would still re-enter the patches.
const snapshot = (source, names) =>
  Object.freeze(Object.fromEntries(names.map((name) => [name, source[name]])));

const fs = Object.freeze({
  ...snapshot(nodeFs, [
    'openSync', 'open', 'closeSync', 'close', 'readSync', 'read', 'writeSync', 'write',
    'readFileSync', 'writeFileSync', 'fstatSync', 'fstat', 'ftruncateSync', 'ftruncate',
    'statSync', 'lstatSync', 'readdirSync', 'mkdirSync', 'rmdirSync', 'unlinkSync',
    'renameSync', 'readlinkSync', 'symlinkSync', 'realpathSync', 'accessSync', 'copyFileSync',
  ]),
  promises: snapshot(nodeFs.promises, [
    'readFile', 'writeFile', 'stat', 'lstat', 'readdir', 'mkdir', 'rmdir', 'unlink',
    'rename', 'readlink', 'symlink', 'realpath', 'access', 'copyFile',
  ]),
});
