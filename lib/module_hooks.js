...
// be handed to the real syscall layer, so they reject it instead.
...
function toOpenFlags(flags) {
  ...
  const name = OPEN_FLAG_NAMES.get(flags);
  if (name === undefined) {
...
function installFsPatches() {
  ...
  fs.lstatSync = function lstatSync(path, options) {
    if (typeof path === 'string') {
      const vfsResult = findVFSForFsLstat(path);
      if (vfsResult !== null) {
  ...
  fs.readlinkSync = function readlinkSync(path, options) {
    if (typeof path === 'string') {
      const vfsResult = findVFSForReadlink(path, options);
      if (vfsResult !== null) {
  ...
  fs.watch = function watch(filename, options, listener) {
    ...
    if (typeof filename === 'string') {
      const vfsResult = findVFSForExistingPath(filename);
      if (vfsResult !== null) {
  ...
  fs.openSync = function openSync(path, flags, mode) {
    if (typeof path === 'string') {
      const vfsResult = findVFSForExistingPath(path);
      if (vfsResult !== null) {
        return vfsResult.vfs.openSync(path, toOpenFlags(flags), mode);
      }
    }
  ...
  fs.open = function open(path, flags, mode, callback) {
    if (typeof path === 'string') {
      const vfsResult = findVFSForExistingPath(path);
      if (vfsResult !== null) {
        return vfsResult.vfs.open(path, toOpenFlags(flags), mode, callback);
      }
    }
