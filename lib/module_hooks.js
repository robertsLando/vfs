...
const { createENOENT, createEBADF, ERR_INVALID_ARG_VALUE } = require('./errors.js');
const {
  getVirtualFd,
  closeFdSync,
  readFdSync,
...
// node accepts a string, a Buffer or a file: URL anywhere it takes a path, and resolves a
// relative one against the cwd - which is the virtual cwd while a VFS with virtualCwd is
// mounted.  Routing on the raw argument sends those forms to the real filesystem while the
// plain absolute string next to them goes to the VFS.
function toRoutablePath(input) {
  ...
  if (Buffer.isBuffer(filename)) {
    ...
    filename = urlToVFSPath(filename);
  ...
  // resolve() reads process.cwd(), which a mounted virtualCwd VFS has already patched
...
function findVFSForRead(filename, options) {
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    const vfs = activeVFSList[i];
    if (vfs.shouldHandle(normalized)) {
      const probe = vfs.probeSync(normalized);
      if (probe.exists) {
        ...
        // The gate failing for a reason is not the same as a miss.
...
function findVFSForExists(filename) {
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    ...
    if (vfs.shouldHandle(normalized)) {
      const probe = vfs.probeSync(normalized);
      if (vfs.mounted || probe.exists) {
        // the error rides along rather than being thrown: existsSync owes the caller a
        // boolean, while access owes it the reason
        return { vfs, exists: probe.exists, error: probe.error };
      }
    }
  }
  return null;
...
function findVFSForFsStat(filename, options) {
  const normalized = normalizeVFSPath(filename);
  for (let i = 0; i < activeVFSList.length; i++) {
    const vfs = activeVFSList[i];
    if (vfs.shouldHandle(normalized)) {
      ...
      if (probe.exists) {
        try {
          const stats = vfs.statSync(normalized, options);
          return { vfs, stats };
        } catch (e) {
          if (vfs.mounted) {
            throw e;
...
// lstat has to describe the link itself, so it cannot share findVFSForFsStat:
// that one calls statSync, which follows the link. A provider that does not
// model links inherits Provider.lstatSync, whose default is statSync, so this
// stays correct for them.
function findVFSForFsLstat(filename, options) {
  const normalized = normalizeVFSPath(filename);
  for (let i = 0; i < activeVFSList.length; i++) {
    const vfs = activeVFSList[i];
    if (!vfs.shouldHandle(normalized)) {
    ...
    // no existsSync pre-check: it follows the link, so a dangling symlink - the case
    // lstat exists for - would read as absent
    try {
      return { vfs, stats: vfs.lstatSync(normalized, options) };
    } catch (e) {
      if (vfs.mounted) {
        throw e;
      }
...
// readlink must reach the provider. Answering it through realpathSync cannot
// tell a link from an ordinary file, so every existing path looked like a
// symlink pointing at itself and EINVAL was unreachable.
function findVFSForReadlink(filename, options) {
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    ...
    // a readlink target may well not exist - that is what a dangling symlink is
    try {
      return { vfs, target: vfs.readlinkSync(normalized, options) };
    } catch (e) {
      if (vfs.mounted) {
        throw e;
      }
...
// Which VFS owns this path?  Overlay mounts only claim paths they actually
// hold, so anything else falls through to the real fs.  Used by watch and by
// the fd-opening patches.
// shouldHandle already runs the provider existence check for an overlay mount, so ownership
// is the whole question here - findVFSForPath answers it.
function findVFSForExistingPath(filename) {
  const vfs = findVFSForPath(normalizeVFSPath(filename));
  return vfs === null ? null : { vfs };
}
...
// fd-taking fs members the VFS does not route.  A virtual fd reaching one of these would
// be handed to the real syscall layer, so they reject it instead.  writeFile and appendFile
// are here for their fd overload; a string path argument never matches a virtual fd.
const UNROUTED_FD_SYNC = [
  'writeSync', 'writevSync', 'readvSync', 'ftruncateSync', 'fsyncSync',
  'fdatasyncSync', 'fchmodSync', 'fchownSync', 'futimesSync',
  'writeFileSync', 'appendFileSync',
];
const UNROUTED_FD_ASYNC = [
  'write', 'writev', 'readv', 'ftruncate', 'fsync',
  'fdatasync', 'fchmod', 'fchown', 'futimes',
  'writeFile', 'appendFile',
];

function adoptFsShape(wrapper, original) {
  for (const sym of Object.getOwnPropertySymbols(original)) {
    Object.defineProperty(wrapper, sym, Object.getOwnPropertyDescriptor(original, sym));
  }
...
// node accepts an O_* bitmask wherever it accepts a flag string; providers only compare
// strings, so translate at the boundary.  The pairs mirror node's own stringToFlags.
const {
  O_RDONLY, O_WRONLY, O_RDWR, O_CREAT, O_TRUNC, O_APPEND, O_EXCL, O_SYNC,
} = require('node:fs').constants;

// the bits that decide which flag string a provider needs; everything else node accepts
// (O_CLOEXEC, O_NOFOLLOW, O_DIRECTORY, O_NONBLOCK, ...) is advisory for a virtual file
const OPEN_FLAG_MASK = O_RDONLY | O_WRONLY | O_RDWR | O_CREAT | O_TRUNC | O_APPEND | O_EXCL | O_SYNC;

const OPEN_FLAG_NAMES = (() => {
...
function toOpenFlags(flags) {
  if (typeof flags !== 'number') {
    return flags;   // a string, or undefined for VirtualFileSystem.openSync's own default
  }

  // advisory bits (O_CLOEXEC, O_NOFOLLOW, O_DIRECTORY, O_NONBLOCK, ...) do not change
  // the access mode a provider cares about, so match on the access/create bits alone
  const name = OPEN_FLAG_NAMES.get(flags & OPEN_FLAG_MASK);
  if (name === undefined) {
    throw new ERR_INVALID_ARG_VALUE('flags', flags, 'is not an open mode a virtual file supports');
  }
  return name;
}
...
function installFsPatches() {
  ...
  fs.readFileSync = function readFileSync(path, options) {
    if (getVirtualFd(path)) {
      throw createEBADF('read', path);
    }
    const routable = toRoutablePath(path);
    if (routable !== null) {
      const vfsResult = findVFSForRead(routable, options);
      if (vfsResult !== null) {
  ...
  fs.realpathSync = function realpathSync(path, options) {
    const routable = toRoutablePath(path);
    if (routable !== null) {
      const vfsResult = findVFSForRealpath(routable);
      if (vfsResult !== null) {
  ...
  fs.lstatSync = function lstatSync(path, options) {
    const routable = toRoutablePath(path);
    if (routable !== null) {
      const vfsResult = findVFSForFsLstat(routable, options);
      if (vfsResult !== null) {
  ...
  fs.statSync = function statSync(path, options) {
    const routable = toRoutablePath(path);
    if (routable !== null) {
      const vfsResult = findVFSForFsStat(routable, options);
      if (vfsResult !== null) {
        return vfsResult.stats;
      }
    }
  ...
  fs.readdirSync = function readdirSync(path, options) {
    const routable = toRoutablePath(path);
    if (routable !== null) {
      const vfsResult = findVFSForReaddir(routable, options);
      if (vfsResult !== null) {
  ...
  fs.existsSync = function existsSync(path) {
    const routable = toRoutablePath(path);
    if (routable !== null) {
      const vfsResult = findVFSForExists(routable);
      if (vfsResult !== null) {
  ...
  fs.readlinkSync = function readlinkSync(path, options) {
    const routable = toRoutablePath(path);
    if (routable !== null) {
      const vfsResult = findVFSForReadlink(routable, options);
      if (vfsResult !== null) {
        return vfsResult.target;
      }
    }
  ...
  fs.accessSync = function accessSync(path, mode) {
    ...
    if (routable !== null) {
      ...
      if (vfsResult !== null) {
        if (!vfsResult.exists) {
          throw vfsResult.error ?? createENOENT('access', routable);
        }
        return;
      }
    }
  ...
  fs.access = function access(path, mode, callback) {
    ...
    if (routable !== null) {
      const vfsResult = findVFSForExists(routable);
      if (vfsResult !== null) {
        const err = vfsResult.exists ? null : (vfsResult.error ?? createENOENT('access', routable));
  ...
  fs.readFile = function readFile(path, options, callback) {
    ...
    if (getVirtualFd(path)) {
      if (callback) process.nextTick(callback, createEBADF('read', path));
      return;
    }
    const routable = toRoutablePath(path);
    if (routable !== null) {
  ...
  fs.createReadStream = function createReadStream(path, options) {
    if (getVirtualFd(path) || getVirtualFd(options?.fd)) {
      throw createEBADF('read', typeof path === 'number' ? path : options.fd);
    }
    const routable = toRoutablePath(path);
    if (routable !== null) {
      try {
  ...
  fs.readlink = function readlink(path, options, callback) {
    ...
    if (routable !== null) {
      try {
        const vfsResult = findVFSForReadlink(routable, options);
        if (vfsResult !== null) {
          if (callback) process.nextTick(callback, null, vfsResult.target);
          return;
        }
      } catch (err) {
        if (callback) process.nextTick(callback, err);
  ...
  fs.watch = function watch(filename, options, listener) {
    ...
    const routable = toRoutablePath(filename);
    if (routable !== null) {
      const vfsResult = findVFSForExistingPath(routable);
      if (vfsResult !== null) {
        return vfsResult.vfs.watch(routable, options, listener);
      }
    }
  ...
  fs.watchFile = function watchFile(filename, options, listener) {
    ...
    if (routable !== null) {
      const vfsResult = findVFSForExistingPath(routable);
      if (vfsResult !== null) {
        return vfsResult.vfs.watchFile(routable, options, listener);
      }
    }
  ...
  fs.unwatchFile = function unwatchFile(filename, listener) {
    ...
    if (routable !== null) {
      const vfsResult = findVFSForExistingPath(routable);
      if (vfsResult !== null) {
        vfsResult.vfs.unwatchFile(routable, listener);
        return;
      }
    }
  ...
  //
  // openSync/open resolve a path to a VFS; the rest route on the fd itself.  fd.js
  // reserves a real descriptor per handle, so a virtual fd is a number no real fd holds.
  //
  // The fall-through branches forward `arguments` untouched: node picks its overload from
  // arguments.length, so re-spelling the signature would break fs.open(path, cb) and
  // fs.read(fd, cb) for real descriptors, process-wide, for every consumer.
  ...
  fs.openSync = function openSync(path, flags, mode) {
    ...
    if (routable !== null) {
      const vfsResult = findVFSForExistingPath(routable);
      if (vfsResult !== null) {
        return vfsResult.vfs.openSync(routable, toOpenFlags(flags), mode);
      }
    }
  ...
  fs.open = function open(path, flags, mode, callback) {
    ...
    if (routable !== null) {
      ...
      if (vfsResult !== null) {
        let openFlags;
        try {
          openFlags = toOpenFlags(flags);
        } catch (err) {
          // a callback API must not throw at the call site for a bad flag
        ...
        return vfsResult.vfs.open(routable, openFlags, mode, callback);
      }
    }
  ...
  fs.close = function close(fd, callback) {
    if (getVirtualFd(fd)) {
      return closeFd(fd, callback);
    }
    return originalClose.apply(fs, arguments);
  };
  ...
  fs.read = function read(fd, buffer, offset, length, position, callback) {
    if (getVirtualFd(fd)) {
      return readFd(fd, buffer, offset, length, position, callback);
    }
  ...
  fs.fstatSync = function fstatSync(fd, options) {
    ...
    return originalFstatSync.apply(fs, arguments);
  };

  const originalFstat = fs.fstat;
  fs.fstat = function fstat(fd, options, callback) {
    if (getVirtualFd(fd)) {
      return fstatFd(fd, options, callback);
    }
    return originalFstat.apply(fs, arguments);
  };

  // node hangs metadata off these (util.promisify reads kCustomPromisifyArgs off fs.read);
  // a replacement that drops it changes the shape for real descriptors, process-wide
  for (const [name, original] of [
    ['openSync', originalOpenSync], ['open', originalOpen],
    ['closeSync', originalCloseSync], ['close', originalClose],
    ['readSync', originalReadSync], ['read', originalRead],
    ['fstatSync', originalFstatSync], ['fstat', originalFstat],
  ]) {
    adoptFsShape(fs[name], original);
  }

  // Only the family above knows what a virtual fd is.  Every other fd-taking member would
  // pass the number straight to the real syscall layer, so refuse it here instead.
  for (const name of UNROUTED_FD_SYNC) {
    const original = fs[name];
    const syscall = name.slice(0, -'Sync'.length);
    fs[name] = adoptFsShape(function(fd) {
      if (getVirtualFd(fd)) {
  ...
  for (const name of UNROUTED_FD_ASYNC) {
    const original = fs[name];
    fs[name] = adoptFsShape(function(fd) {
      if (getVirtualFd(fd)) {
        const callback = findCallback(arguments);
        if (callback === undefined) {
          throw createEBADF(name, fd);
        }
        process.nextTick(callback, createEBADF(name, fd));
        return undefined;
      }
      return original.apply(fs, arguments);
    }, original);
