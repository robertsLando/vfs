...
function findVFSForFsLstat(filename) {
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    ...
    try {
      return { vfs, stats: vfs.lstatSync(normalized) };
    } catch (e) {
      if (vfs.mounted) {
        throw e;
      }
...
function findVFSForExistingPath(filename) {
  // fs.open pays this on every call in the process, mounted or not
  if (activeVFSList.length === 0) {
    return null;
  }
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    const vfs = activeVFSList[i];
    // shouldHandle already runs the provider existence check for an overlay mount;
    // repeating it here cost a second provider lookup - a second query on SQLite - per open
    if (vfs.shouldHandle(normalized)) {
      return { vfs };
    }
  }
...
function installFsPatches() {
  ...
  fs.lstatSync = function lstatSync(path, options) {
    ...
    if (routable !== null) {
      ...
      const vfsResult = findVFSForFsLstat(routable);
      if (vfsResult !== null) {
  ...
  fs.statSync = function statSync(path, options) {
    ...
    if (routable !== null) {
      rejectBigIntStats(options);
      const vfsResult = findVFSForFsStat(routable);
      if (vfsResult !== null) {
        return vfsResult.stats;
      }
    }
  ...
  fs.accessSync = function accessSync(path, mode) {
    ...
    if (routable !== null) {
      ...
      if (vfsResult !== null) {
        if (!vfsResult.exists) {
          throw createENOENT('access', routable);
        }
        return;
      }
    }
  ...
  fs.access = function access(path, mode, callback) {
    ...
    if (routable !== null) {
      const vfsResult = findVFSForExists(routable);
      if (vfsResult !== null) {
        const err = vfsResult.exists ? null : createENOENT('access', routable);
