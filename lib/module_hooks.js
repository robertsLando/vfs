  ...
  Module._resolveFilename = function(request, parent, isMain, options) {
    ...
    if (!isAbsolute(request) && request[0] !== '.') {
      ...
      for (let i = 0; i < activeVFSList.length; i++) {
        ...
        while (currentDir !== lastDir) {
          const pkgDir = normalizeVFSPath(resolve(currentDir, 'node_modules', packageName));
          if (vfs.shouldHandle(pkgDir) && vfs.internalModuleStat(pkgDir) === 1) {
            if (packageSubpath === '.') {
              ...
              if (vfs.internalModuleStat(pjsonPath) === 0) {
                try {
                  ...
                  if (parsed.main) {
                    const mainPath = normalizeVFSPath(resolve(pkgDir, parsed.main));
                    if (vfs.internalModuleStat(mainPath) === 0) return mainPath;
                    const withExt2 = tryExtensions(vfs, mainPath);
              ...
              const subResolved = normalizeVFSPath(resolve(pkgDir, packageSubpath));
              if (vfs.internalModuleStat(subResolved) === 0) return subResolved;
              const withExt2 = tryExtensions(vfs, subResolved);
              if (withExt2) return withExt2;
            }
          }
          lastDir = currentDir;
